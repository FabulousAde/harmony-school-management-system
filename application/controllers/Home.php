...
class Home extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        // $this->session->set_flashdata('flash_message', get_phrase(''.$user_details['name'].'-'));
        
        // if((current_url() != site_url() || current_url() != site_url('home')) && ($user_details['status'] != true || $this->session->userdata('user_login') != true)) {
        
        if($this->session->userdata('user_login') == true && ((current_url() != site_url() && current_url() != site_url('home')) && ($user_details['status'] != true))) {
    ...
    public function home() {
        $page_data['top_courses'] = $this->crud_model->get_top_courses()->result_array();
        $page_data['top_selling_courses'] = $this->crud_model->get_top_selling_courses()->result_array();
        $page_data['top_activities'] = $this->crud_model->get_top_activities()->row_array();
    ...
    public function courses() {
        ...
        if (isset($_GET['category']) && !empty($_GET['category'] && $_GET['category'] != "all") && !empty($_GET['ucategory'])) {
            // $selected_category_id = $this->crud_model->get_category_id($_GET['category']);
            $selected_category_id = $_GET['ucategory'];
        }
    ...
    public function cbt($param1 = '', $param2 = '') {
        ...
        if($param1 == '') {
            ...
            $page_data['all_cbt'] = $this->crud_model->get_usercbtexams();
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        } elseif ($param1 == 'desc') {
            $exam_id = ($param2 - 224);
            $page_data['page_name'] = "cbt_desc";
            $page_data['page_title'] = get_phrase("exam_description");
            $page_data['cbt'] = $this->crud_model->get_cbtexams(($param2 - 224))->row_array();
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        }
    }
    ...
    public function study_history() {
        ...
        // $total_rows = $this->crud_model->study_history($this->session->userdata('user_id'))->num_rows();
        // $config = array();
        // $config = pagintaion($total_rows, 3);
        // $config['base_url']  = site_url('home/study_history');
        // $this->pagination->initialize($config);
        $page_data['per_page']   = $config['per_page'];
        $page_data['page_name']  = "study_history";
        $page_data['page_title'] = get_phrase('study_history');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function my_assignments() {
        ...
        $page_data['page_title'] = get_phrase('my_assignments');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    
    public function assignments_expand($param1 = "") {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        if($param1 == ""){
        ...
        $page_data['assignment_details'] = $param1;
        $page_data['page_title'] = get_phrase('my_assignments_expand');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    
    public function activiities($param1 = '') {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        if($param1 == ''){
            ...
            $page_data['page_title'] = get_phrase('school_activities');
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        }else{
            $page_data['per_page']   = $config['per_page'];
            $page_data['page_name']  = "view_actiities";
            $page_data['all_activities'] = $this->crud_model->get_activities(($param1-28));
            $page_data['page_title'] = get_phrase('school_activities');
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        }
    }
    
    public function my_results($param1 = '') {
         if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }
        if($param1 == ''){
            ...
            $page_data['user'] = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
            $page_data['all_result_logs'] = $this->user_model->get_result_logs($page_data['user']);
            $page_data['page_title'] = get_phrase('my_results');
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        } else {
            $page_data['per_page']   = $config['per_page'];
            $page_data['page_name']  = "results_inner";
            $page_data['user'] = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
            // $log = $this->user_model->get_result_logs($page_data['user'], $param1)->row_array();
            // $page_data['log'] = $log;
            $page_data['id'] = $param1;
            // $page_data['results'] = $this->user_model->get_results($log['id'], $log['class_category']);
            $page_data['page_title'] = get_phrase('my_results');
            $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
        }
    }
    ...
    public function pastquestion() {
        if ($this->session->userdata('user_login') != true || get_settings('use_past_question') != '1') {
            redirect(site_url('home'), 'refresh');
        }
        $page_data['page_name'] = "past_question_home";
        $page_data['page_title'] = get_phrase('past_questions');
        $page_data['user'] = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
        $this->load->view('frontend/'.get_frontend_settings('theme').'/index', $page_data);
    }
    ...
    public function paystack_payment_success($user_id = "", $refId = '', $amount_paid = "") {
        $this->crud_model->enrol_student($user_id);
        $this->crud_model->course_purchase($user_id, 'paystack', $amount_paid, $refId);
        $this->session->set_userdata('cart_items', array());
        $this->session->set_flashdata('flash_message', get_phrase('payment_successfully_done'));
        redirect('home', 'refresh');
    }
    ...
    public function submit_assignment($assignment_id) {
        if($this->session->userdata('user_login') != 1 || $assignment_id == "" || $assignment_id == 0){
            redirect('home/my_assignments', 'refresh');
        }
        
        $this->crud_model->save_assignment($this->session->userdata('user_id'), $assignment_id);
        $this->session->set_flashdata('flash_message', get_phrase('assignments_submitted_successfully'));
        redirect('home/my_assignments', 'refresh');
    }
    ...
    public function instructor($param1 = '') {
        if ($this->session->userdata('admin_login')) {
            redirect(site_url('admin'), 'refresh');
        } elseif ($this->session->userdata('user_login')) {
            redirect(site_url('user'), 'refresh');
    ...
    public function submit_quiz() {
        ...
        $this->crud_model->add_study($total_correct_answers, $this->input->post('lesson_id'), count($quiz_questions));
    ...
    public function cbt_questions($param1) {
        // $details = $this->input->post();
        // echo $param1;
        $check = $this->crud_model->checkCbtEnrolled($param1);
        if(!$check) {
            $exams = $this->crud_model->get_exams($param1);
            // $resp = $this->crud_model->get_all_qcategories($details);
            $page_data['exams'] = $exams;
            $page_data['exam_id'] = $param1;
            $page_data['list_length'] = $exams->num_rows();
            $this->db->insert('cbt_enrol', array('user_id' => $this->session->userdata('user_id'), 'exam_id' => $param1, 'time_enrolled' => strtotime(date('D, d-M-Y'))));
            $this->load->view('frontend/'.get_frontend_settings('theme').'/cbt_question_views', $page_data);
        }
    }

    public function submit_cexams() {
        $submitted_quiz_info = array();
        $container = array();
        $exam_id = $this->input->post('lesson_id');

        // echo $exam_id;
        $exam_lessons = $this->crud_model->get_exams($exam_id)->result_array();
        // echo count($exam_lessons);
        $total_questions = $total_gotten = 0;

        foreach ($exam_lessons as $key => $elesson) {
            // echo 'my key  '.$key;
            $exam_questions = $this->crud_model->get_examquestions($elesson['id'])->result_array();
            // $quiz = $this->db->get_where('lesson', array('course_id' => $exam_id))->result_array();
            $total_correct_answers = 0;
            foreach ($exam_questions as $exam_question) {
                // echo 'okay '.$exam_question['id'];
                $total_questions++;
                $submitted_answer_status = 0;
                $correct_answers = json_decode($exam_question['correct_answers']);
                $submitted_answers = array();
                foreach ($this->input->post($elesson['id'].'_'.$exam_question['id']) as $each_submission) {
                    if (isset($each_submission)) {
                        array_push($submitted_answers, $each_submission);

                        // echo 'here';
                        // echo $each_submission.'<br>';
                        // echo 'key: '.$key;
                    }
                }
                sort($correct_answers);
                sort($submitted_answers);
                $study_data['user_id'] = $this->session->userdata('user_id');
                $study_data['exam_id'] = $exam_id;
                $study_data['question_id'] = $exam_question['id'];
                if ($correct_answers == $submitted_answers) {
                    $submitted_answer_status = 1;
                    $total_correct_answers++;
                    $study_data['gotten'] = '1';
                }
                $study_data['date_added'] = strtotime(date('D, d-M-Y'));
                $this->db->insert('cbt_studies', $study_data);
                $total_gotten = $total_correct_answers;
                $container = array(
        ...
        $data['total_gotten'] = $total_correct_answers;
        $data['total_questions'] = $total_questions;
        $enrol = $this->db->get_where('cbt_enrol', array('user_id' => $this->session->userdata('user_id'), 'exam_id' => $exam_id))->row_array();
        $this->crud_model->enrol_edit($enrol['id'], $data);
        
        // // $this->crud_model->add_study($total_correct_answers, $this->input->post('lesson_id'), count($exam_questions));
        // $page_data['quiz_info'] = $this->db->get_where('lesson', array('id' => $exam_id))->row_array();
        // $page_data['submitted_quiz_info']   = $submitted_quiz_info;
        // $page_data['total_correct_answers'] = $total_correct_answers;
        // $page_data['total_questions'] = count($exam_questions);
        // $this->load->view('frontend/'.get_frontend_settings('theme').'/questions_result', $page_data);
    }

    public function checkIfEnroll($param1) {
    ...
    public function past_questions() {
        $details = $this->input->post();
        $resp = $this->crud_model->get_all_qcategories($details);
        $page_data['cc_list'] = $resp;
        $page_data['list_length'] = count($resp);
        // echo $resp;
        $this->load->view('frontend/'.get_frontend_settings('theme').'/question_views', $page_data);
    }
    ...
    public function submit_questions() {
        ...
        $quiz_id = $this->input->post('lesson_id');
        $quiz_questions = $this->crud_model->get_quiz_questions($quiz_id)->result_array();
        // $quiz = $this->db->get_where('lesson', array('course_id' => $quiz_id))->result_array();
        $total_correct_answers = 0;
        foreach ($quiz_questions as $quiz_question) {
            $submitted_answer_status = 0;
            $correct_answers = json_decode($quiz_question['correct_answers']);
        ...
        // $this->crud_model->add_study($total_correct_answers, $this->input->post('lesson_id'), count($quiz_questions));
        $page_data['quiz_info'] = $this->db->get_where('lesson', array('id' => $quiz_id))->row_array();
        $page_data['submitted_quiz_info']   = $submitted_quiz_info;
        $page_data['total_correct_answers'] = $total_correct_answers;
        $page_data['total_questions'] = count($quiz_questions);
        $this->load->view('frontend/'.get_frontend_settings('theme').'/questions_result', $page_data);
