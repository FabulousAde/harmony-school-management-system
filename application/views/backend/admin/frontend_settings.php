        ...
        <div class="card">
            <div class="card-body">
                ...
                <form class="required-form" action="<?php echo site_url('admin/frontend_settings/frontend_update'); ?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="banner_instructor_text"><?php echo get_phrase('banner_instructor_text'); ?><span class="required">*</span></label>
                        <input type="text" name = "banner_instructor_text" id = "banner_instructor_text" class="form-control" value="<?php echo get_frontend_settings('banner_instructor_text');  ?>" required>
                    </div>
...
<div class="card">
    <div class="card-body">
        <h4 class="mb-3 header-title"><?php echo get_phrase('partners_settings');?></h4>
        <div class="row col-xl-12 col-md-12 col-sm-12 col-lg-12">
            <?php for ($i = 0; $i < 6; $i++): ?>
                <div class="col-xl-4 col-md-4 col-sm-4 col-lg-4">
                    <div class="card">
                        <div class="card-body">
                            <div class="col-xl-12">
                                <h4 class="mb-3 header-title"><?php echo get_phrase('update_partner_image_'.($i + 1)); ?></h4>
                                <div class="row justify-content-center">
                                    <form action="<?php echo site_url('admin/partners/parters_image_update/'.($i + 1)); ?>" method="post" enctype="multipart/form-data" style="text-align: center;">
                                        <div class="form-group mb-2">
                                            <div class="wrapper-image-preview">
                                                <div class="box" style="width: 250px;">
                                                    <div class="js--image-preview" style="background-image: url(<?php echo base_url('uploads/partners/partner'.($i + 1).'.jpg');?>); background-color: #F5F5F5;"></div>
                                                    <div class="upload-options">
                                                        <label for="partner_image_<?= ($i + 1); ?>" class="btn"> <i class="mdi mdi-camera"></i> <?php echo get_phrase('upload_partner_'.($i + 1)); ?> <br> <small>(113 × 36)</small> </label>
                                                        <input id="partner_image_<?= ($i + 1); ?>" style="visibility:hidden;" type="file" class="image-upload" name="partner_image_<?= ($i + 1); ?>" accept="image/*">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <button type="submit" class="btn btn-primary btn-block"><?php echo get_phrase('upload_partner_'.($i + 1)); ?></button>
                                    </form>
                                </div>
                            </div>
                        </div>
