                      ...
                      <form action="<?php echo site_url('login/register'); ?>" method="post">
                          <div class="content-box">
                              <div class="basic-group">
                                  ...
                                  <!-- <div class="form-group">
                                      <label for="select_class"><span class="input-field-icon"><i class="fas fa-award"></i></span> <?php echo get_phrase('select_class'); ?>:</label>
                                      <select name="category" class="category_option form-control selection_height" style="height: 47px;" required>
                                        <option value=""><?php echo get_phrase("select_class_category"); ?></option>
                                        <option value="preschool"><?php echo get_phrase("preschool"); ?></option>
...
var preschoolclasses = ['select_class', 'playgroup_1', 'playgroup_2', 'nursery_1', 'nursery_2'];
  var primaryclasses = ["select_class", "primary_1", "primary_2", "primary_3", "primary_4", "primary_5", "primary_6"];
  var docclasses = {"preschool": preschoolclasses, "primary": primaryclasses};
  $(document).ready(function () {
    $('.category_option').on('change', function () {
      var option = $('.category_option').val();
      var form = "";
      for (var i = 0;  i < docclasses[option].length; i++) {
        // var newoption = docclasses[option][i];
        form = form + "<option value="+change_name(docclasses[option][i])+">"+change_name(docclasses[option][i], 0)+"</option>";
      }
      $('.class_options').html('<div class="form-group"><select name="class_options" class="form-control selection_height" style="height: 47px;" required>'+form+'</select></div>');
    });
  });
  ...
  function change_name(data, r) {
    var initvalue = (r == 0) ? data.replace("_", " ") : data;
